...
import org.lorislab.p6.deployment.DefaultBpmnReader;
...
public class ModelSerializerServiceTest {

    @EJB
    private ModelSerializerService service;

    private static final Logger LOGGER = LoggerFactory.getLogger(ModelSerializerServiceTest.class);
    ...
    public void serializerTest() throws Exception {
        ...
        Files.walkFileTree(Paths.get(BPMN_RESOURCE_DIR), new SimpleFileVisitor<Path>() {
            ...
            public FileVisitResult visitFile(Path file, BasicFileAttributes attrs) throws IOException {
                try {
                    LOGGER.info("Start serializer for process: {}", file.toString());
                    ...
                    for (JAXBElement<? extends TRootElement> e : definitions.getRootElements()) {
                        if (e.getValue() instanceof TProcess) {
                            RuntimeProcess process = reader.create((TProcess) e.getValue());
                            LOGGER.info("Process start serializer: {} id: {}", file, process);
                            byte[] data = service.toByte(process);
                            RuntimeProcess process2 = service.fromByte(data, RuntimeProcess.class);
                            LOGGER.info("Process finished serializer: {} id: {}", file, process);
                            assertThat(process2).isEqualToComparingFieldByFieldRecursively(process);
                        }
                    }                    
                } catch (Exception ex) {
